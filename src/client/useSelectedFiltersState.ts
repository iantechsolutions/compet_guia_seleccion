...
// export default function useSelectedFiltersState(questions: QuestionFilter[], initialUrl: URL, onUrlChanged: (url: URL) => unknown) {
...
//             onUrlChanged(new URL(window!.location.href, window!.location.origin))
...
//     function goBack(filters: SelectedFilters, callback?: () => unknown) {
//         if (!window) return
...
//                 filters.push({
//                     key: question.key,
//                     values: values?.split(',').filter(valueIndex => !!valueFromIndex(index, parseInt(valueIndex))?.key).map(valueIndex => valueFromIndex(index, parseInt(valueIndex)).key) || [],
//                     questionIndex: index++
//                 })
//             }
//         }
