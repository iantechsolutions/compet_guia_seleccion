...
import {
	...
	shouldIgnoreQuestionOption,
} from "../functions/filter_products";
...
import NavKeysController, {
	type NavKeyEvent,
	Action as NavKeyEventAction,
} from "nav-keys";
import {
	type Entry,
	addEntry,
	updateCurrentState,
} from "../client/saveLocalState";
...
export default function QuestionsViewWrapper(props: QuestionsViewProps) {
	...
	const [initialFilters, setInitialFilters] = useState<SelectedFilters | null>(
		null,
	);
	...
	return (
		<QuestionsView
			{...props}
			onShowInitialPage={() => setStarted(false)}
...
export function QuestionsView({
	...
	function getNextIndex(carry = 0) {
		const lastFilter = selectedFilters[selectedFilters.length - 1];

		const nextIndex = (lastFilter ? lastFilter.questionIndex + 1 : 0) + carry;

		if (nextIndex >= flatQuestions.length) {
			return nextIndex;
		}

		const ignore = shouldIgnoreQuestion(
			filteredProducts,
			selectedFilters,
			flatQuestions[nextIndex],
		);
	...
	function forward() {
		...
		setSelectedFilters([
			...selectedFilters,
			{ ...nextFilterValue, questionIndex: questionIndex, question },
		]);
	...
	return (
		<div>
			<div
				className="fixed bottom-0 top-0 left-0 w-[0px]"
				id="height-snitcher"
			/>
			...
			<div className="min-h-[100vh]" id="set-height">
				...
				<Question
					...
					question={{
						...question,
						values: question.values.filter(
							(value) =>
								!shouldIgnoreQuestionOption(filteredProducts, question, [
									value.key,
								]),
						),
					}}
					onChange={setNextFilterValue}
					values={nextFilterValue?.values}
				/>

				<div className="container">
					{!isLast &&
						!showProducts &&
						filteredProducts.length < SHOW_MAX_PRODUCTS && (
							<a
								href="!#"
							...
							>
								Mostrar {filteredProducts.length} productos
							</a>
						)}
					{!isLast &&
						showProducts &&
					...
					{(isLast ||
						(showProducts && filteredProducts.length < SHOW_MAX_PRODUCTS)) && (
						<>
			...
			<div className="sticky bottom-0 left-0 right-0 py-[12px] px-[20px] shadow-[2px_0_10px_-3px_rgba(0,0,0,0.3)] flex justify-between">
				...
				<div className="flex items-center">
					<button
						type="buttom"
						className="rounded-full border p-1 flex gap-2 items-center pr-2"
						onClick={() => {
